"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .managementdomaingroup import ManagementDomainGroup
from orq_ai_sdk.types import BaseModel, UNSET_SENTINEL
from pydantic import model_serializer
from typing import Optional
from typing_extensions import NotRequired, TypedDict


class ManagementDomainTypedDict(TypedDict):
    r"""ManagementDomain describes a permission domain that can be granted
    to a management key. Verbs are derived from id + readable/writable.
    """

    id: NotRequired[str]
    r"""Stable domain identifier (e.g. \"api-key\", \"budget\"). Used as the
    key in ManagementKey.access and as the verb prefix in resolved
    permissions (e.g. budget.list, budget.view, budget.create).
    """
    display_name: NotRequired[str]
    r"""Human-readable label for the dashboard."""
    group: NotRequired[ManagementDomainGroup]
    readable: NotRequired[bool]
    r"""Whether this domain can be granted read access."""
    writable: NotRequired[bool]
    r"""Whether this domain can be granted write access."""


class ManagementDomain(BaseModel):
    r"""ManagementDomain describes a permission domain that can be granted
    to a management key. Verbs are derived from id + readable/writable.
    """

    id: Optional[str] = None
    r"""Stable domain identifier (e.g. \"api-key\", \"budget\"). Used as the
    key in ManagementKey.access and as the verb prefix in resolved
    permissions (e.g. budget.list, budget.view, budget.create).
    """

    display_name: Optional[str] = None
    r"""Human-readable label for the dashboard."""

    group: Optional[ManagementDomainGroup] = None

    readable: Optional[bool] = None
    r"""Whether this domain can be granted read access."""

    writable: Optional[bool] = None
    r"""Whether this domain can be granted write access."""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["id", "display_name", "group", "readable", "writable"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m
