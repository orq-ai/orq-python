"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .expression import Expression, ExpressionTypedDict
from .guardrailref import GuardrailRef, GuardrailRefTypedDict
from datetime import datetime
from orq_ai_sdk.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from orq_ai_sdk.utils import FieldMetadata, PathParamMetadata
import pydantic
from pydantic import model_serializer
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class GuardrailRuleGetRequestTypedDict(TypedDict):
    guardrail_rule_id: str
    r"""The ID of the guardrail rule"""


class GuardrailRuleGetRequest(BaseModel):
    guardrail_rule_id: Annotated[
        str, FieldMetadata(path=PathParamMetadata(style="simple", explode=False))
    ]
    r"""The ID of the guardrail rule"""


class GuardrailRuleGetResponseBodyTypedDict(TypedDict):
    r"""Guardrail rule retrieved successfully"""

    id: str
    created_at: datetime
    created_by_id: str
    display_name: str
    enabled: bool
    project_id: str
    timeout: int
    updated_at: datetime
    updated_by_id: str
    description: NotRequired[str]
    expression: NotRequired[ExpressionTypedDict]
    guardrails: NotRequired[Nullable[List[GuardrailRefTypedDict]]]


class GuardrailRuleGetResponseBody(BaseModel):
    r"""Guardrail rule retrieved successfully"""

    id: Annotated[str, pydantic.Field(alias="_id")]

    created_at: datetime

    created_by_id: str

    display_name: str

    enabled: bool

    project_id: str

    timeout: int

    updated_at: datetime

    updated_by_id: str

    description: Optional[str] = None

    expression: Optional[Expression] = None

    guardrails: OptionalNullable[List[GuardrailRef]] = UNSET

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["description", "expression", "guardrails"])
        nullable_fields = set(["guardrails"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields
                and (self.__pydantic_fields_set__.intersection({n}))  # pylint: disable=no-member
            )

            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val

        return m


try:
    GuardrailRuleGetResponseBody.model_rebuild()
except NameError:
    pass
