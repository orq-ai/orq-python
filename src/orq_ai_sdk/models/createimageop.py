...
CreateImagePluginsTypedDict = TypeAliasType(
    "CreateImagePluginsTypedDict",
    Union[
        PIIRedactionPluginAutoTypedDict,
...
CreateImagePlugins = TypeAliasType(
    "CreateImagePlugins",
...
class CreateImageRequestBodyTypedDict(TypedDict):
    ...
    r"""Timeout configuration to apply to the request. If the request exceeds the timeout, it will be retried or fallback to the next model if configured."""
    plugins: NotRequired[List[CreateImagePluginsTypedDict]]
...
class CreateImageRequestBody(BaseModel):
    ...
    plugins: Optional[List[CreateImagePlugins]] = None
    r"""Request-scoped transforms applied to the text exchanged with the model. Currently supports `pii_redaction`, which replaces PII with placeholders before the provider sees it and restores the original values in the response."""
